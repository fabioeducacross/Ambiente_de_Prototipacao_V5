  ...
  <Transition name="slide">
    <aside v-if="modelValue" class="tz-drawer" role="dialog" aria-modal="true" :aria-labelledby="'drawerTitle'">
      ...
      <header class="drawer-header">
        <h2 class="drawer-title" id="drawerTitle">
          {{ mode === 'enviar' ? 'Enviar missão em lote' : 'Pausar missão em lote' }}
        </h2>
...
const allSelected = computed(() =>
  eligibleStudents.value.length > 0 &&
  eligibleStudents.value.every(s => selectedIds.has(s.id))
)

const someSelected = computed(() =>
  eligibleStudents.value.some(s => selectedIds.has(s.id))
)
...
function toggleAll () {
  if (allSelected.value) {
    eligibleStudents.value.forEach(s => selectedIds.delete(s.id))
  } else {
    eligibleStudents.value.forEach(s => selectedIds.add(s.id))
  }
}
