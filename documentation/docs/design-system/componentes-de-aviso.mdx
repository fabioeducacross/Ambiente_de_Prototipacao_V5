...
**Quando usar:** orientação ou estado informativo dentro de um <GlossaryTerm id="drawer">painel lateral (drawer)</GlossaryTerm>. Compacto, sem título, texto único.
...
<GlossaryTerm id="drawer">Drawers</GlossaryTerm> são contextos **compactos e focados em tarefa**. O Bootstrap `.alert` e o <GlossaryTerm id="BAlert">`BAlert`</GlossaryTerm> foram projetados para ocupar o nível de página — têm espaçamento generoso, bordas, e constroem hierarquia visual de página. Dentro de um drawer, esse peso visual é excessivo e compete com o conteúdo principal. O <GlossaryTerm id="drawer-hint">`drawer-hint`</GlossaryTerm> usa padding reduzido (`10px 12px`), borda-raio menor e fundo com 8% de opacidade via <GlossaryTerm id="color-mix">`color-mix()`</GlossaryTerm> — quase imperceptível, mas presente o suficiente para contextualizar a orientação.
...
O <GlossaryTerm id="drawer-hint">`drawer-hint`</GlossaryTerm> não é feedback de uma ação — é orientação prévia. O Carbon Design System (IBM) define seu componente equivalente (<GlossaryTerm id="callout">`Callout`</GlossaryTerm>) explicitamente como persistente:
...
O <GlossaryTerm id="drawer-hint">`drawer-hint`</GlossaryTerm> é um componente de **orientação antes da ação**, não de **feedback após a ação**. O Carbon é explícito sobre isso: seu `Callout` permite apenas `informational` e `warning` — `success` e `error` são exclusivos da <GlossaryTerm id="inline-notification">Inline Notification</GlossaryTerm>, disparada por resultado de ação. Dentro de um drawer de envio, um `--danger` vermelho geraria confusão — o usuário interpretaria como "algo deu errado" quando na verdade há apenas uma atenção necessária.
...
Copiar para o componente que usar (<GlossaryTerm id="scoped-styles">scoped</GlossaryTerm>):
...
- Sempre começar com ícone `info` (primário e neutro) ou `warning` (variante warning) do <GlossaryTerm id="material-symbols">Material Symbols</GlossaryTerm>
...
- Não usar hex inline — apenas `var(--token)` (<GlossaryTerm id="design-tokens">design tokens</GlossaryTerm>)
...
**Porta de produção:** `components/missions/GuidesLimitAlert.vue` (BootstrapVue 2 → <GlossaryTerm id="bootstrap-vue-next">bootstrap-vue-next</GlossaryTerm>)
...
- Usar sempre <GlossaryTerm id="bootstrap-vue-next">bootstrap-vue-next</GlossaryTerm> (`BAlert`) — não escrever `<div class="alert">` manualmente em páginas
- `:model-value="true"` para manter visível (sem fechar)
- Conteúdo sempre dentro de `.alert-body`
- Para lógica multi-layout, encapsular em componente dedicado (padrão <GlossaryTerm id="GuidesLimitAlert">`GuidesLimitAlert`</GlossaryTerm>)
...
| Hint para explicar o que o botão já deveria dizer | Melhorar o label do botão |
...
Em março/2026 foi feito um benchmark comparativo com os principais Design Systems do mercado para validar as decisões arquiteturais do <GlossaryTerm id="drawer-hint">`drawer-hint`</GlossaryTerm>. Abaixo os achados por sistema.

### Carbon Design System (IBM) — `Callout`

O componente mais próximo do <GlossaryTerm id="drawer-hint">`drawer-hint`</GlossaryTerm>. A definição do Carbon é quase idêntica ao nosso uso:
...
Especificação mais detalhada de <GlossaryTerm id="semantic-variant">variantes semânticas</GlossaryTerm>. Define 5 níveis:
...
Traduzido: se o hint existe para explicar o botão, o botão está mal rotulado.
